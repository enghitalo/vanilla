...
# Machine-readable tally on STDERR (keeps stdout = the scorecard markdown). The
# gate reads `fail=` — it fails ONLY on 🔴 real conformance regressions, never on
# ⚪ blocked (no-response, which can be transient socket timing on a noisy runner
# or a tracked backend edge case).
sys.stderr.write(f"SCORECARD_COUNTS pass={n_pass} fail={n_fail} blocked={n_block}\n")
