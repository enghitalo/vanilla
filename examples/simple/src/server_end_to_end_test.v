...
// asserts by script index (position = identity).
...
fn test_server_end_to_end() ! {
	...
	for i, c in cases {
		...
		assert out.conns[i].frames.len == 1, c.name
		assert out.conns[i].frames[0] == c.want, '${c.name}: got ${out.conns[i].frames[0].bytestr()}'
	}
