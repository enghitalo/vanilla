...
bytes-in/bytes-out handlers, allocation-free matching on the hot path) while
being safe to put in front of real traffic.
...
request buffer (zero-copy); call `.to_string(req.buffer)` to materialize it.
...
request, simple and allocation-free; a trie would trade that for O(path-length)
at the cost of "no magic".
...
| `responses.v` | JSON/text response builders + JSON string escaping |
...
# GET  /users
# POST /users
# GET  /users/:id/get?format=json&pretty=true
# GET  /users/:id/posts/:post_id
```

## Benchmarks (wrk -t8 -c128, loopback)
