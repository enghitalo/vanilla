...
fn tls_handshake_step(mut conn TlsConn, epoll_fd int, fd int, active_conns &Counter, mut sessions map[int]&TlsConn) bool {
...
@[direct_array_access; manualfree]
fn handle_writable_fd_tls(epoll_fd int, fd int, active_conns &Counter, mut sessions map[int]&TlsConn) {
...
@[manualfree]
fn tls_send_or_park(epoll_fd int, fd int, limits Limits, active_conns &Counter, mut sessions map[int]&TlsConn, mut conn TlsConn, resp []u8) {
	mut sent := 0
	for sent < resp.len {
...
@[manualfree]
fn sweep_timeouts_tls(epoll_fd int, active_conns &Counter, mut sessions map[int]&TlsConn) {
