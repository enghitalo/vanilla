...
fn handle_readable_plain(request_handler fn ([]u8, int) ![]u8, epoll_fd int, fd int, limits Limits, counter &Counter, active_conns &Counter, mut conns map[int]&ConnState) {
...
@[manualfree]
fn handle_writable_plain(epoll_fd int, fd int, active_conns &Counter, mut conns map[int]&ConnState) {
...
@[manualfree]
fn close_conn(epoll_fd int, fd int, active_conns &Counter, mut conns map[int]&ConnState) {
	if cs := conns[fd] {
		unsafe {
			if cs.read_buf.len > 0 {
				cs.read_buf.free()
