...
fn test_port_zero_resolves_to_real_port() ! {
	...
	assert srv.port > 0, 'Server.port must carry the kernel-assigned port'
	assert socket.local_port(srv.socket_fd) == srv.port
...
fn test_port_zero_io_uring_listeners_share_one_port() ! {
	$if linux {
		...
		assert srv.port > 0
		assert srv.listener_fds.len == 4
		for fd in srv.listener_fds {
			assert socket.local_port(fd) == srv.port, 'every SO_REUSEPORT listener must hold the SAME resolved port'
		}
		for fd in srv.listener_fds {
			socket.close_socket(fd)
		}
...
fn test_explicit_port_is_untouched() ! {
	...
	assert srv.port == 18990
	assert socket.local_port(srv.socket_fd) == 18990
	for fd in srv.listener_fds {
		socket.close_socket(fd)
	}
}
