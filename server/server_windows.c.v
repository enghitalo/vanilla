...
pub fn run_iocp_backend(srv Server, mut threads []thread) {
	if srv.socket_fd < 0 {
		return
	}
	...
	for i in 0 .. n_workers {
		ports[i] = iocp.create_iocp(1) or {
			...
			socket.close_socket(srv.socket_fd)
			exit(1)
		}
	}
	...
	for i in 0 .. n_workers {
		counter := srv.inflight[i] // this worker's own in-flight counter
		t := spawn win_worker(ports[i], srv.handler, srv.make_state, srv.limits, counter,
			srv.active_conns)
		threads[i] = t // caller-visible handle (same contract as the other backends)
		workers[i] = t
	}
	println('listening on http://localhost:${srv.port}/ (IOCP)')
	...
	win_accept_loop(srv.socket_fd, ports, srv.limits, srv.active_conns, srv.draining)
...
// per OS so the all-platform facade (server.c.v) needs no
...
fn run_selected_backend(srv Server, mut threads []thread) {
	match srv.io_multiplexing {
		.iocp {
			run_iocp_backend(srv, mut threads)
		}
	}
}
